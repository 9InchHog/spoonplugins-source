...
public interface ZulrahConfig extends Config {
    ...
    default FontType fontType() {return FontType.SMALL;}
    ...
    @Range(max = 255, min = 0)
    @Units("%")
    default int fillAlpha() {return 15;}
    ...
    default boolean phaseTickCounter() {return false;}
    ...
    default boolean attackTickCounter() {return false;}
    ...
    @Alpha
    default Color tickCounterColors() {return Color.WHITE;}
    ...
    default boolean totalTickCounter() {return false;}
    ...
    default boolean displayZulrahTile() {return false;}
    ...
    @Alpha
    default Color zulrahTileColor() {return Color.LIGHT_GRAY;}
    ...
    @ConfigItem(
            ...
            description =
                    "Displays an overlay showing the correct prayer to use for the entirity of the Zulrah fight"
                            + "Changes color dependent on whether or not you're praying correctly or not",
            position = 0,
    ...
    @ConfigItem(
            ...
            description =
                    "Marks the correct prayer to use in the prayer book to use for the entirity of the Zulrah fight"
                            + "Changes color dependent on whether or not you're praying correctly or not",
            position = 1,
            section = fightHelperSection
    )
    default boolean prayerMarker() {return true;}

    @ConfigItem(
            name = "Prayer Conservation",
            keyName = "prayerConservation",
            description = "Displays text over your head showing when it's safe to turn off your overheads"
                    + "Overlay gets displayed when Zulrah is not actively targeting you and your overheads are on",
            position = 2,
            section = fightHelperSection
    )
    ...
    default boolean standLocations() {return true;}
    ...
    @Alpha
    default Color standAndNextTileColor() {return Color.GRAY;}
    ...
    @Alpha
    default Color standTileColor() {return Color.CYAN;}
    ...
    @Alpha
    default Color nextTileColor() {return Color.GREEN;}
    ...
    default boolean stallLocations() {return false;}
    ...
    @Alpha
    default Color stallTileColor() {return Color.PINK;}
    ...
    @ConfigItem(
            ...
            description = "Overlay: Displays Zulrah's phases details on an overlay"
                    + "Tile: Displays Zulrah's phases details on tiles",
            position = 0,
            section = phaseHelperSection
    )
    default ZulrahConfig.DisplayType phaseDisplayType() {return DisplayType.OFF;}

    @ConfigItem(
            name = "Display Mode",
            keyName = "phaseDisplayMode",
            description = "Current: Only displays the current Zulrah phase"
                    + "Next: Only displays the next Zulrah phase",
            position = 1,
            section = phaseHelperSection
    )
    default DisplayMode phaseDisplayMode() {return DisplayMode.BOTH;}

    @ConfigItem(
            name = "Rotation Name",
            keyName = "phaseRotationName",
            description = "Requires: Display Type ('Overlay' or 'Both')"
                    + "Displays text above InfoBox overlay showing the rotation name or unidentified",
            position = 2,
            section = phaseHelperSection
    )
    default boolean phaseRotationName() {return false;}
    ...
    default boolean phaseHats() {return true;}

    @ConfigItem(
            name = "Tags",
            keyName = "phaseTags",
            description = "Tags each Zulrah phase on the tile overlay with:"
                    + "[Current] = Current Zulrah phase" + "[Next] = Definite next Zulrah phase"
                    + "[P. Next] = Potentially Zulrah's next phase",
            position = 4,
            section = phaseHelperSection
    )
    default boolean phaseTags() {return true;}
    ...
    @ConfigItem(
            ...
            description =
                    "Displays an overlay showing how long Zulrah has been alive in minutes:seconds format"
                            + "Timer resets on Zulrah death and/or leaving of the instance of any fashion",
            position = 0,
    ...
    @ConfigItem(
            ...
            description = "Remove Att. Op.: Removes the 'Attack' option from all the active Snakelings"
                    + "Entity Hider: Hides all the active Snakelings",
            position = 1,
            section = miscellaneousSection
    )
    default ZulrahConfig.SnakelingSettings snakelingSetting() {return SnakelingSettings.OFF;}
    ...
    default Keybind snakelingMesHotkey() {return Keybind.NOT_SET;}
    ...
    default boolean displayToxicClouds() {return false;}
    ...
    @Alpha
    default Color toxicCloudsColor() {return Color.GREEN;}
    ...
    default boolean displayProjectiles() {return false;}
    ...
    @Alpha
    default Color projectilesColor() {return Color.LIGHT_GRAY;}

    enum SnakelingSettings {
        OFF("Off"),
        MES("Deprio. Att. Op."),
        ENTITY("Entity Hider");
