...
public class SceneOverlay extends Overlay {
                ...
                switch (config.phaseDisplayMode()) {
                    case CURRENT:
                        data.getCurrentZulrahNpc().ifPresent((npc) -> {
                            zulrahLocationsGrouped
                                    .put(npc.getZulrahLocation(), new MutablePair<>("Current", npc));
                        });
                        break;
                    case NEXT:
                        data.getNextZulrahNpc().ifPresent((npc) -> {
                            zulrahLocationsGrouped
                                    .put(npc.getZulrahLocation(), new MutablePair<>(getZulrahNextString(), npc));
                        });
                        break;
                    case BOTH:
                        data.getCurrentZulrahNpc().ifPresent((npc) -> {
        ...
        if (zulrahNpc != null) {
            ...
            if (basePoint != null) {
                ...
                if (this.config.phaseHats()) {
                    BufferedImage icon = skillIconManager.getSkillImage(zulrahNpc.getType().getSkill(), config.fontType() != FontType.BOLD);
    ...
    private void renderPrayerConservation(Graphics2D graphics) {
        if (config.prayerConservation()) {
            ...
            if (player != null && (plugin.getZulrahNpc().getInteracting() == null || plugin.getZulrahNpc().getInteracting() != client.getLocalPlayer())
    ...
    private void renderZulrahTicks(Graphics2D graphics) {
        if (config.phaseTickCounter() || config.attackTickCounter()) {
            ...
            sb = sb
                    .append(config.phaseTickCounter() && plugin.getPhaseTicks() >= 0 ? plugin.getPhaseTicks() : "")
                    .append(config.phaseTickCounter() && config.attackTickCounter() && plugin.getPhaseTicks() >= 0 && plugin.getAttackTicks() >= 0 ? " : " : "")
                    .append(config.attackTickCounter() && plugin.getAttackTicks() >= 0 ? plugin.getAttackTicks() : "");
