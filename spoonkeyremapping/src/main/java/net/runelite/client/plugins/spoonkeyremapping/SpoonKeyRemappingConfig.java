...
public interface SpoonKeyRemappingConfig extends Config
{
	@ConfigSection(
	...
	@ConfigItem(
		...
		keyName = "noEsc",
		name = "No Escape",
		description = "Makes it so that the ESC key does not reset chat back to Press to Enter Chat"
	)
	default boolean noEsc()
	{
		return false;
	}
	...
	@ConfigItem(
		position = 23,
		keyName = "exraKeybinds",
		name = "Extra Keybinds",
		description = "Lets you set additional keybinds for F keys<br><u>Format</u><br>f1:1,f1:q,f2:w"
	)
	default String exraKeybinds()
	{
		return "";
	}
}
