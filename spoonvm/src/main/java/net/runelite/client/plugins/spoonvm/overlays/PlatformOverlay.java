...
public class PlatformOverlay extends Overlay {
    ...
    @Inject
    private PlatformOverlay(Client client, SpoonVMPlugin plugin, Constants constants, SpoonVMConfig config, ItemManager itemManager) {
        this.client = client;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (config.platformHighlight() != SpoonVMConfig.PlatformTimerStyle.OFF && plugin.platforms.size() > 0) {
             for(SpoonVMObjects vmObj : plugin.platforms){
                Polygon poly = vmObj.obj.getCanvasTilePoly();
                if (config.platformHighlight() == SpoonVMConfig.PlatformTimerStyle.BOTH) {
                    OverlayUtil.renderPolygon(graphics, poly, rgbMod.apply(11, vmObj.ticks));
                    Point textLocation = vmObj.obj.getCanvasTextLocation(graphics, Integer.toString(vmObj.ticks), 0);
                    Objects.requireNonNull(this.config);
                    renderTextLocation(graphics, textLocation, Integer.toString(vmObj.ticks), Color.WHITE, this.config::txtOutline);
                } else if (config.platformHighlight() == SpoonVMConfig.PlatformTimerStyle.TILE) {
                    OverlayUtil.renderPolygon(graphics, poly, rgbMod.apply(11, vmObj.ticks));
                } else if (config.platformHighlight() == SpoonVMConfig.PlatformTimerStyle.TEXT) {
                    Point textLocation = vmObj.obj.getCanvasTextLocation(graphics, Integer.toString(vmObj.ticks), 0);
                    Objects.requireNonNull(this.config);
                    renderTextLocation(graphics, textLocation, Integer.toString(vmObj.ticks), Color.WHITE, this.config::txtOutline);
                }
